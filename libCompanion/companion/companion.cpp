...
// Private Methods                             //
//\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\//
void Companion::resize_image_equal(cv::Mat &img1, cv::Mat &img2)
{
    int x;
	int y;
	...
	resize(img1, img1, cv::Size(x, y));
	resize(img2, img2, cv::Size(x, y));
}

void Companion::resize_image(cv::Mat &img, int size_x, int size_y)
{
	resize(img, img, cv::Size(size_x, size_y));
}
